...
| **Session logging / rest timer**  | Live session on `/workouts/:id?session=1`; finish opens `/history/:sessionId`. History calendar at `/history` (≥3 months back; day bars = primary muscle per set from exercise categories, 50/50 when exactly two muscles in a session). Rest timer during session. |
